...
#' the creation and training of a classifier an object of class [EmbeddedText] or [LargeDataSetForTextEmbeddings] on the one hand
#' and a [factor] on the other hand are necessary.
#'
#' The object of class [EmbeddedText] or [LargeDataSetForTextEmbeddings]  contains the numerical text
#' representations (text embeddings) of the raw texts generated by an object of class
#' [TextEmbeddingModel]. For supporting large data sets it is recommended to use [LargeDataSetForTextEmbeddings] instead of
#' [EmbeddedText].
#'
#' The `factor` contains the classes/categories for every
#' text. Missing values (unlabeled cases) are supported and can be used for pseudo labeling.
#'
#' For predictions an object of class
#' [EmbeddedText] or [LargeDataSetForTextEmbeddings] has to be used which was created with the same
...
TEClassifierRegular <- R6::R6Class(
  ...
  public = list(
    ...
    #' estimated iota reliability of the second generation for the final model
    #' for every fold for the test data. Please note that the model is estimated without
    ...
    #' @param hidden `vector` containing the number of neurons for each dense layer.
    #' The length of the vector determines the number of dense layers. If you want no dense layer,
    #' set this parameter to `NULL`.
    #' @param rec `vector` containing the number of neurons for each recurrent layer.
    #' The length of the vector determines the number of dense layers. If you want no dense layer,
    #' set this parameter to `NULL`.
    #' @param rec_type `string` Type of the recurrent layers. `rec_type="gru"` for
    #' Gated Recurrent Unit and `rec_type="lstm"` for Long Short-Term Memory.
    #' @param rec_bidirectional `bool` If `TRUE` a bidirectional version of the recurrent
    #' layers is used.
    #' @param attention_type `string` Choose the relevant attention type. Possible values
    #' are `fourier` and `multihead`.
    #' @param self_attention_heads `int` determining the number of attention heads
    #' for a self-attention layer. Only relevant if `attention_type="multihead"`
    #' @param repeat_encoder `int` determining how many times the encoder should be
    #' added to the network.
    #' @param intermediate_size `int` determining the size of the projection layer within
    #' a each transformer encoder.
    #' @param add_pos_embedding `bool` `TRUE` if positional embedding should be used.
    #' @param encoder_dropout `int` ranging between 0 and lower 1, determining the
    #' dropout for the dense projection within the encoder layers.
    #' @param dense_dropout `int` ranging between 0 and lower 1, determining the
    #' dropout between dense layers.
    #' @param rec_dropout `int` ranging between 0 and lower 1, determining the
    #' dropout between bidirectional recurrent layers.
    #' @param recurrent_dropout `int` ranging between 0 and lower 1, determining the
    ...
    #' @param data_folds `int` determining the number of cross-fold
    #' samples.
    #' @param data_val_size `double` between 0 and 1, indicating the proportion of cases of each class
    #' which should be used for the validation sample during the estimation of the model.
    #' The remaining cases are part of the training data.
    #' @param balance_class_weights `bool` If `TRUE` class weights are
    ...
    #' @param balance_sequence_length `bool` If `TRUE` sample weights are
    #' generated for the length of sequences based on the frequencies of the training data with the method
    #' Inverse Class Frequency'. If `FALSE` each sequences length has the weight 1.
    #' @param use_sc `bool` `TRUE` if the estimation should integrate
    #' synthetic cases. `FALSE` if not.
    #' @param sc_method `vector` containing the method for generating
    #' synthetic cases. Possible are `sc_method="adas"`, `sc_method="smote"`,
    #' and `sc_method="dbsmote"`.
    #' @param sc_min_k `int` determining the minimal number of k which is used
    #' for creating synthetic units.
    #' @param sc_max_k `int` determining the maximal number of k which is used
    #' for creating synthetic units.
    #' @param use_pl `bool` `TRUE` if the estimation should integrate
    #' pseudo-labeling. `FALSE` if not.
    #' @param pl_max_steps `int` determining the maximum number of steps during
    #' pseudo-labeling.
    #' @param pl_anchor `double` between 0 and 1 indicating the reference
    #' point for sorting the new cases of every label. See notes for more details.
    #' @param pl_max `double` between 0 and 1, setting the maximal level of
    #' confidence for considering a case for pseudo-labeling.
    #' @param pl_min `double` between 0 and 1, setting the minimal level of
    #' confidence for considering a case for pseudo-labeling.
    ...
    #' @param newdata Object of class [TextEmbeddingModel] or [LargeDataSetForTextEmbeddings]
    ...
    #' @param require_compressed `TRUE` if a compressed version of the embeddings are
    #' necessary. Compressed embeddings are created by an object of class [TEFeatureExtractor].
    #' @return `TRUE` if the underlying [TextEmbeddingModel] is the same.
    ...
    #' @return This method does nothing returns. It raises an error
    #' if
    #'
    #' * the object is `NULL`
    ...
    #--------------------------------------------------------------------------
    #' @description Method for checking if provided text embeddings must be
    #' compressed via a [TEFeatureExtractor] before processing.
    #' @param text_embeddings Object of class [EmbeddedText], [LargeDataSetForTextEmbeddings],
    #' `array` or `datasets.arrow_dataset.Dataset`.
    ...
    save = function(dir_path, folder_name) {
      ...
      if (self$model_config$use_fe == TRUE) {
        save_to_disk(object= self$feature_extractor,
  ...
  private = list(
    ...
    check_and_adjust_target_levels=function(data_targets){
      if(sum(levels(data_targets)%in%self$model_config$target_levels)!=self$model_config$n_categories){
        warning(
          paste("data_targets contains levels that are not defined for the classifier",
