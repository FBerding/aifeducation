...
create_roberta_model <- function(
  ...
  .Deprecated("aife_transformer_maker$make(\"roberta\")$create()")

  aife_transformer_maker$make("roberta")$create(
...
#' @family Deprecated transformer
#' @export
train_tune_roberta_model <- function(
    ml_framework = aifeducation_config$get_framework()[1],
    output_dir,
    model_dir_path,
    raw_texts,
    ...
    pytorch_trace = 1,
    pytorch_safetensors = TRUE) {
  .Deprecated("aife_transformer_maker$make(\"roberta\")$train()")

  aife_transformer_maker$make("roberta")$train(
    ml_framework = ml_framework,
