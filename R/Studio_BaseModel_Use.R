  ...
  shiny::tagList(
    bslib::page_sidebar(
      ...
      bslib::navset_card_underline(
        bslib::nav_panel(
          title = "Description",
          Description_UI(id = shiny::NS(id, "BaseModel_Desc"))
        ),
        bslib::nav_panel(
          title = "Training",
        ...
        bslib::nav_panel(
          ...
          Tokenize_Encode_Decode_UI(id = shiny::NS(id, "BaseModel_TokEnDe"))
        )
      )
    )
  )
...
#' @noRd
#'
BaseModel_Use_Server <- function(id, log_dir, volumes) {
        ...
        if ("try-error" %in% class(model) == FALSE) {
          if ("BaseModelCore" %in% class(model)) {
            ...
            display_errors(
              ...
              size = "l",
              easy_close = TRUE,
              error_messages = "The file does not contain an object of class BaseModelCore."
            )
            return(NULL)
          }
        } else {
