#'@title Embedded text
#'@description Object of class `R6` which stores the text embeddings
#'generated by an object of class [TextEmbeddingModel]. The text embeddings
#'are stored within memory/RAM. In the case of a high number of documents the
#'data may not fit into memory/RAM. Thus, please use this object only for a small
#'sample of texts. In general, it is recommended to use an object of class [LargeDataSetForTextEmbeddings]
#'which can deal with any number of texts.
#'@return Returns an object of class [EmbeddedText]. These objects are used
#'for storing and managing the text embeddings created with objects of class [TextEmbeddingModel].
#'Objects of class [EmbeddedText] serve as input for objects of class
...
EmbeddedText<-R6::R6Class(
  ...
  public = list(
    ...
    #'@return `list` contains all saved information about the underlying
    ...
    #'case a [feature extractor][TEFeatureExtractor] was used the number of features is smaller as the
    #'original number of features. To receive the original number of features (the number of features before
    #'applying a [feature extractor][TEFeatureExtractor]) you can use the method `get_original_features`
    #'of this class.
    ...
    #'[feature extractor][TEFeatureExtractor]) is used or before a
    #'[feature extractor][TEFeatureExtractor]) is applied.
    ...
    #'@return Returns `TRUE` if the number of dimensions was reduced by a [feature extractor][TEFeatureExtractor].
    #'If not return `FALSE`.
    ...
    #--------------------------------------------------------------------------
    #'@description Method setting information on the [feature extractor][TEFeatureExtractor] that
    #'was used to reduce the number of dimensions of the text embeddings. This
    #'information should only be used if a [feature extractor][TEFeatureExtractor] was applied.
    ...
    #'@return Returns a `list` with information on the [feature extractor][TEFeatureExtractor].
    #'If no [feature extractor][TEFeatureExtractor] was used it returns `NULL`.
    ...
    #'@return Returns the number of rows of the text embeddings which represent
    #'the number of cases.
    n_rows=function(){
