...
TextEmbeddingClassifierNeuralNet <- R6::R6Class(
  ...
  public = list(
    ...
    #' @field reliability ('list()')\cr List for storing central reliability measures of the last training.
    #' \itemize{
    ...
    #' \item{\code{reliability$raw_iota_objects: }List containing all iota_object generated with the package
    #' \code{iotarelr}
    #' for every fold at the start and the end of the last training.
    #' \itemize{
    #' \item{\code{reliability$raw_iota_objects$iota_objects_start: }List of objects with class \code{iotarelr_iota2}
    #' containing the
    ...
    #' set to \code{NULL}.}
    #' \item{\code{reliability$raw_iota_objects$iota_objects_end: }List of objects with class \code{iotarelr_iota2}
    #' containing the
    ...
    #' \item{\code{reliability$raw_iota_objects$iota_objects_start_free: }List of objects with class
    #' \code{iotarelr_iota2} containing the
    #' estimated iota reliability of the second generation for the baseline model
    #' for every fold.
    #' If the estimation of the baseline model is not requested, the list is
    ...
    #' \item{\code{reliability$raw_iota_objects$iota_objects_end_free: }List of objects with class \code{iotarelr_iota2}
    #' containing the estimated iota reliability of the second generation for the final model
    #' for every fold. Depending of the requested training method, these values
    #' refer to the baseline model, a trained model on the basis of balanced
    #' synthetic cases, balanced pseudo-labeling or a combination of balanced
    ...
    #' }
    #' }
    #' \item{\code{reliability$iota_object_start: }Object of class \code{iotarelr_iota2} as a mean of the individual
    #' objects
    #' for every fold. If the estimation of the baseline model is not requested, the list is
    #' set to \code{NULL}.}
    #' \item{\code{ reliability$iota_object_start_free: }Object of class \code{iotarelr_iota2} as a mean of the
    #' individual objects
    #' for every fold. If the estimation of the baseline model is not requested, the list is
    #' set to \code{NULL}.
    #' Please note that the model is estimated without
    #' forcing the Assignment Error Matrix to be in line with the assumption of weak superiority.}
    #' \item{\code{reliability$iota_object_end: }Object of class \code{iotarelr_iota2} as a mean of the individual
    #' objects for every fold.
    ...
    #' synthetic cases, balanced pseudo-labeling or a combination of balanced
    #' synthetic cases and pseudo-labeling.}
    #' \item{\code{reliability$iota_object_end_free: }Object of class \code{iotarelr_iota2} as a mean of the individual
    #' objects for every fold.
    #' Depending on the requested training method, this object
    #' refers to the baseline model, a trained model on the basis of balanced
    #' synthetic cases, balanced pseudo-labeling or a combination of balanced
    ...
    #' @param ml_framework \code{string} Framework to use for training and inference. \code{ml_framework="tensorflow"}
    #'   for 'tensorflow' and \code{ml_framework="pytorch"} for 'pytorch'
    #' @param name \code{Character} Name of the new classifier. Please refer to common name conventions. Free text can
    #'   be used with parameter \code{label}.
    #' @param label \code{Character} Label for the new classifier. Here you can use free text.
    ...
    #' @param hidden \code{vector} containing the number of neurons for each dense layer. The length of the vector
    #'   determines the number of dense layers. If you want no dense layer, set this parameter to \code{NULL}.
    #' @param rec \code{vector} containing the number of neurons for each recurrent layer. The length of the vector
    #'   determines the number of dense layers. If you want no dense layer, set this parameter to \code{NULL}.
    #' @param attention_type \code{string} Choose the relevant attention type. Possible values are \code{"fourier"} and
    #'   \code{multihead}.
    #' @param self_attention_heads \code{integer} determining the number of attention heads for a self-attention layer.
    #'   Only relevant if \code{attention_type="multihead"}
    #' @param repeat_encoder \code{int} determining how many times the encoder should be added to the network.
    #' @param intermediate_size \code{int} determining the size of the projection layer within a each transformer
    #'   encoder.
    #' @param add_pos_embedding \code{bool} \code{TRUE} if positional embedding should be used.
    #' @param encoder_dropout \code{double} ranging between 0 and lower 1, determining the dropout for the dense
    #'   projection within the encoder layers.
    #' @param dense_dropout \code{double} ranging between 0 and lower 1, determining the dropout between dense layers.
    #' @param rec_dropout \code{double} ranging between 0 and lower 1, determining the dropout between bidirectional gru
    #'   layers.
    #' @param recurrent_dropout \code{double} ranging between 0 and lower 1, determining the recurrent dropout for each
    #'   recurrent layer. Only relevant for keras models.
    #' @param optimizer Object of class \code{keras.optimizers}.
    #' @return Returns an object of class \link{TextEmbeddingClassifierNeuralNet} which is ready for training.
    ...
    train = function(data_embeddings,
      ...
      if (use_bpl == TRUE & use_bsc == FALSE & use_baseline == FALSE) {
        ...
        pgr_max_value <- data_n_test_samples * (use_baseline + use_bsc + use_bpl * bpl_max_steps) +
          use_baseline + use_bsc + use_bpl * bpl_max_steps + 1
      }
      ...
      viable_cases <- base::intersect(
        x = rownames(data_embeddings$embeddings),
        names(data_targets)
      )
      ...
      if (sum(freq_check_eval) > 0) {
        ...
        stop(paste(
          "Categories", cat, "have absolute frequencies below 4.",
      ...
      dimnames(data_pbl) <- list(
        folds = c(
          paste0("fold_", seq(from = 1, to = folds$n_folds, by = 1)),
          "final_train"
        ),
        bpl_steps = NULL
      )
      ...
      data_bsc_train <- matrix(
        data = 0,
        nrow = folds$n_folds + 1,
        ncol = length(categories)
      )
      rownames(data_bsc_train) <- c(
        paste0("fold_", seq(from = 1, to = folds$n_folds, by = 1)),
        "final"
      )
      ...
      for (iter in 1:folds$n_folds) {
        ...
        if (use_baseline == TRUE |
          ...
          # Train model
          tmp_history <- private$basic_train(
            embedding_train = embeddings_all[names_targets_labeled_train_train, , ],
          ...
          test_predictions <- self$predict(
            newdata = embeddings_all[names_targets_labaled_test, , ],
            verbose = keras_trace,
            batch_size = batch_size
          )
          ...
          test_res <- get_coder_metrics(
            true_values = targets_labeled_test,
          ...
          if (use_bsc == TRUE | use_bpl == TRUE) {
            iota_objects_start[iter] <- list(iotarelr::check_new_rater(
              true_values = targets_labeled_test,
              assigned_values = test_pred_cat,
              free_aem = FALSE
            ))
            iota_objects_start_free[iter] <- list(iotarelr::check_new_rater(
              true_values = targets_labeled_test,
              assigned_values = test_pred_cat,
              free_aem = TRUE
            ))
        ...
        if (use_bsc == TRUE) {
          ...
          syn_cases <- get_synthetic_cases(
            embedding = embeddings_all[names_targets_labeled_train, , ],
          ...
          tmp_history <- private$basic_train(
            embedding_train = embeddings_all_and_synthetic[names_targets_labeled_train_train, , ],
        ...
        if (use_bpl == TRUE) {
          ...
          while (step <= bpl_max_steps & added_cases_train > 0) {
            ...
            est_remaining_data <- self$predict(
              newdata = embeddings_all[names_unlabeled, , ],
              verbose = keras_trace,
              batch_size = batch_size
            )
            ...
            for (cat in new_categories_names) {
              ...
              if (bpl_balance == TRUE) {
                ...
                # of all categories
                names_final_new_categories <- append(
                  x = names_final_new_categories,
            ...
            tmp_history <- private$basic_train(
              embedding_train = pseudo_label_embeddings_all[names(targets_labeled_and_pseudo), , ],
            ...
            val_predictions <- self$predict(
              newdata = pseudo_label_embeddings_all[names(pseudo_label_targets_labeled_val), , ],
              verbose = keras_trace,
              batch_size = batch_size
            )
            ...
            val_res <- get_coder_metrics(
              true_values = pseudo_label_targets_labeled_val,
              predicted_values = val_pred_cat
            )

            # Predict test targets
            test_predictions <- self$predict(
              newdata = embeddings_all[names(pseudo_label_targets_labeled_test), , ],
              verbose = keras_trace,
              batch_size = batch_size
            )
            test_pred_cat <- test_predictions$expected_category
            ...
            test_res <- get_coder_metrics(
              true_values = pseudo_label_targets_labeled_test,
              predicted_values = test_pred_cat
            )
        ...
        iota_objects_end[iter] <- list(iotarelr::check_new_rater(
          true_values = targets_labeled_test,
          assigned_values = test_pred_cat,
          free_aem = FALSE
        ))
        iota_objects_end_free[iter] <- list(iotarelr::check_new_rater(
          true_values = targets_labeled_test,
          assigned_values = test_pred_cat,
          free_aem = TRUE
        ))
        self$reliability$standard_measures_end[iter] <- list(calc_standard_classification_measures(
          true_values = targets_labeled_test,
          predicted_values = test_pred_cat
        ))
      ...
      if (use_bsc == TRUE) {
        ...
        #     file="debug.RData")
        syn_cases <- get_synthetic_cases(
          embedding = embeddings_all[names(targets_labeleld_all), , ],
  ...
  private = list(
    ...
    create_model_pytorch = function(features,
      ...
      return(py$TextEmbeddingClassifier_PT(
        features = as.integer(features),
        times = as.integer(times),
        hidden = if (!is.null(hidden)) {
          as.integer(hidden)
        } else {
          NULL
        },
    ...
    create_model_keras = function(features,
      ...
      if (n_rec > 0 | repeat_encoder > 0) {
        model_input <- keras$layers$Input(
          shape = list(as.integer(times), as.integer(features)),
          name = "input_embeddings"
        )
      } else {
        model_input <- keras$layers$Input(
          shape = as.integer(times * features),
          name = "input_embeddings"
        )
      }
    ...
    basic_train = function(embedding_train,
      ...
      if (length(target_levels_order) > 2) {
        # Multi Class
        output_categories_train <- to_categorical_c(
          class_vector = target_train_transformed,
          n_classes = n_categories
        )
        output_categories_val <- to_categorical_c(
          class_vector = target_val_transformed,
          n_classes = n_categories
        )
        if (is.null(target_test) == FALSE) {
          output_categories_test <- to_categorical_c(
            class_vector = target_test_transformed,
            n_classes = n_categories
          )
        }
      } else {
      ...
      if (private$ml_framework == "tensorflow") {
        ...
        history <- model$fit(
          ...
          validation_data = reticulate::tuple(list(
            x_val = np$array(input_embeddings_val),
