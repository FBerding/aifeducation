...
detect_base_model_type <- function(model) {
  ...
  if (stringi::stri_detect(str = tolower(type_string), regex = "^funnel([:alnum:]*)")) {
    ...
    return("longformer")
  } else if (stringi::stri_detect(str = tolower(type_string), regex = "^xlmroberta([:alnum:]*)")) {
    return("robertaxml")
    ...
    return("modernbert")
  } else if (stringi::stri_detect(str = tolower(type_string), regex = "^albert([:alnum:]*)")) {
    return("albert")
...
ProgressIndicator <- R6::R6Class(
  ...
  public = list(
    ...
    print_step = function(iter = NULL, text_pre = NULL, text_post = NULL) {
      ...
      pre_text <- if (is.null(text_pre)) {
        ""
      } else {
        text_pre
      }
      post_text <- if (is.null(text_post)) {
        ""
      } else {
        paste0(" ", text_post)
      }
