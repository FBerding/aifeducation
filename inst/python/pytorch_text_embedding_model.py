...
class TextEmbeddingModel(torch.nn.Module):
  def __init__(self,base_model,chunks,  emb_layer_min, emb_layer_max,emb_pool_type,pad_value):
    super().__init__()
  ...
  def forward(self,input_ids,attention_mask,token_type_ids=None):
    ...
    n_layers=emb_layer_max-emb_layer_min+1
    ...
    if self.emb_pool_type=="Average":
      ...
      n_elements=torch.unsqueeze(n_layers*torch.sum(attention_mask,dim=1),dim=1).expand(sum_over_layers.size())
      ...
      final_embeddings=torch.sum(cls_tokens,dim=(1))/n_layers
