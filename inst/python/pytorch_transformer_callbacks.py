...
class ReportAiforeducationShiny_PT(transformers.TrainerCallback):
  ...
  def on_step_end(self, args, state, control, **kwargs):
    r.py_update_aifeducation_progress_bar_steps(value=(state.global_step % (state.max_steps/args.num_train_epochs)),total=state.max_steps,title=("Batch/Step: "+str(state.global_step)+"/"+str(int(state.max_steps/args.num_train_epochs))))
