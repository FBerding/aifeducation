...
class BatchNorm_with_Mask(torch.nn.Module):
    ...
    def forward(self, x,seq_len,mask_times,mask_features):
      ...
      if self.training==True:
        ...
        batch_variance=batch_variance/(n_elements-1)
        #Update running mean and variance
        self.running_mean=(1-self.alpha)*self.running_mean+self.alpha*torch.unsqueeze(torch.unsqueeze(batch_mean,dim=0),dim=0)
        self.running_variance=(1-self.alpha)*self.running_variance+self.alpha*torch.unsqueeze(torch.unsqueeze(batch_variance,dim=0),dim=0)*(x_zeros.size(0)/(x_zeros.size(0)-1+self.eps))
        #Scale and shift
