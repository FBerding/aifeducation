...
Next you must install Protocol Buffer Compiler if it is not already on your system.
This compiler is a prerequisite for two python packages ('protbuf' and 'sentencepiece') 
some open source models need to work. These packages are also necessary to update
older open source models to newer version of the library 'transformers'. 
You can find installation instruction
on (https://protobuf.dev/installation/)[https://protobuf.dev/installation/].
...
> Starting with version 1.1.6 the python packages 'protobuf' and 'sentencepiece ' are necessary. If you
...
> After that install the package manually with `reticulate::py_install(c("protobuf","sentencepiece"))` or call
> `aifeducation::update_aifeducation`.
