...
machine. Since natural language processing with neural nets is based on
...
-   torch,
-   keras,
-   tensorflow and its dependencies in the environment "aifeducation".
    
If you would like to use *aifeducation* with other packages or with
...
Now everything is ready to use the package. When you start a new *R*
session, please note that you have to call
`reticulate::use_condaenv(condaenv = "aifeducation")` on a machine with
windows and `reticulate::use_virtualenv(condaenv = "aifeducation")` on
mac and linux to make the python modules available for work.
...
function `set_config_tf_logger()`. You can choose between five levels
