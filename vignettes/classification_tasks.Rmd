...
aifeducation](img_articles/classif_model_hierachy.png){width="100%"}
...
First, the function receives the machine learning framework you chose at
the start of the session. However, you can change this by setting
`ml_framework="tensorflow"` or by `ml_framework="pytorch"`.
...
language or task, you can call the corresponding `train`-method. Use
`aife_transformer_maker` to create a transformer object. In case of a
BERT model this is `aife_transformer_maker$make("bert")$train()`.
...
capacity, please reduce the batch size significantly. We also recommend
to change the usage of memory with `set_config_gpu_low_memory()` at the
beginning of the session if you use *tensorflow* as framework.
...
`ml_framework="tensorflow"` or `ml_framework="pytorch"`. If you do not
change this argument, the framework you chose at the beginning is used.
...
"ProtoNet"](img_articles/classif_fig_protonet_emb_1.png){width="100%"}
...
unlabeled
cases](img_articles/classif_fig_protonet_emb_2.png){width="100%"}
...
including unlabeled
cases](img_articles/classif_fig_protonet_emb_3.png){width="100%"}
...
extractors](img_articles/classif_feature_extractor.png){width="100%"}
...
standard dense layers. If you set `method="conv"` all layers are
