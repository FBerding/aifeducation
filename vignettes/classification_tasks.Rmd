...
`config` represents a list where you decide about the structure of your classifier
...
second with 64 neurons. `rec` also takes a vector of integers determining the number
 ...
 *Adaptive Synthetic Sampling Approach for Imbalanced Learning*. The aim is to 
 ...
 extended data and after training it predicts the classes/categories of the remaining
 ...
 quality of the classifier. Thus, we recommend to include only a small percentage
 (`bpl_inc_ratio`) in every step for the training of the next step. The added 
 cases have an increased weight that increases with every step by 1.
...
Figure 4 illustrates the training loop for the cases that all three options
are set to `TRUE`.

![Figure 4: Overview of the Training Loop for all Options Activiated](classif_fig_04.png){width=100%}
...
classifier<-aifeducation::test_classifier
```
...
```{r, fig.height = 6, fig.width = 7.2, fig.align = "center", fig.cap = "Figure 5: Coding Stream of the Classifier",include = TRUE, eval=TRUE}
library(iotarelr)
iotarelr::plot_iota2_alluvial(aifeducation::test_classifier$reliability$iota_object_end_free)
```
