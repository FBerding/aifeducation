...
example_folders <- setdiff(
  ...
  y = c("examples", "examples/bert_uncased","examples/xml_roberta_fb")
)
...
```{r, include = TRUE, eval = TRUE}
base_model <- BaseModelBert$new()
base_model$configure(
  tokenizer = tokenizer,
...
```{r base_model$train, include = TRUE, eval = TRUE}
base_model$train(
  text_dataset = data_set_reviews_text,
  p_mask = 0.30,
  whole_word = TRUE,
...
base_model$plot_training_history(
  ind_best_model = TRUE
)
```
...
```{r, include = TRUE, eval = TRUE}
# We use t() to improve the readability of the table
t(base_model$get_sustainability_data("training"))
```
...
```{r, include = TRUE, eval = TRUE}
# We use t() to improve the readability of the table
t(base_model$get_sustainability_data("inference"))
```
...
```{r, include = TRUE, eval = TRUE}
# We use t() to improve the readability of the table
t(base_model$get_flops_estimates())
```
...
base_model_bert$create_from_hf(
...
[xlm-roberta-base](https://huggingface.co/FacebookAI/xlm-roberta-base) published by
FacebookAI. It is based on the work by Conneau et al. (2019). 
Download all files into a new folder. Here we store the model in  `"examples/xml_roberta_fb"`.

We first load the base model as described in section 3.

```{r, include = TRUE, eval = TRUE}
base_model <- BaseModelRobertaXML$new()
base_model$create_from_hf(
  model_dir = "examples/xml_roberta_fb",
  tokenizer_dir = "examples/xml_roberta_fb"
)
```

Now we can create the TextEmbeddingModel. At the beginng we have to chose
...
section 3.3). If the text produces more tokens, the last tokens are
...
Our BaseModel can process 512 tokens at maximum (`seq_len_tokens`). Passing 
these values to the `calc_quantiles` method of the tokenizer of our BaseModel reveals the
following quantiles:

```{r, include = TRUE, eval = TRUE}
chunk_quantile = base_model$Tokenizer$calc_quantiles(
  text_dataset = data_set_reviews_text, 
  batch_size = 32L, 
  seq_len_tokens = 512L,
  token_overlap=128L,
...
by our text embedding model we need `r chunk_quantile["90%"]` chunks. To ensure
...
of $512+(4-1)*(512-128)=1664$ tokens of a text.
...
tem$configure(
  model_label = "Text Embedding via RobertaXML",
