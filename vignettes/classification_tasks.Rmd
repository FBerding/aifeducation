...
to the initial model state. The values express the degree in percentage to the initial loss.
A decrease is desired.
The points of the plot shows how many epochs yield a smaller loss compared 
to the loss of the epoch before. They express the percentage of improved epochs on all epochs. 
You can interpret this as a measure of stability.
The automatic calculation selects the learning rate from the longest range of learning rates.
That is, the used learning rate is selected from the mean index position of the highest learning rate
and the index with the strongest decrease of loss. The minimum learning rate is selected from
the mean index position of the lowest learning rate and the index with the strongest decrease of loss.
...
classifier_prototype$configure(
  ...
  cls_pooling_features = 20,
  cls_pooling_type = "Max",
  projection_type = "PairwiseOrthogonalDense",
