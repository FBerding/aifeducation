...
example_folders=setdiff(x=list.dirs(path="examples"),
...
```{r, include = TRUE, eval = TRUE}
tokenizer$get_tokenizer_statistics()
```
...
```{r, include = TRUE, eval = TRUE}
tokenizer$get_sustainability_data("training")
```
...
```{r, include = TRUE, eval = TRUE}
base_model$get_sustainability_data("training")
```
...
```{r, include = TRUE, eval = TRUE}
base_model$get_sustainability_data("inference")
```
...
plot. Classifier's loss (`"loss"`), accuracy (`"accuracy"`), and 
...
First we plot the training history.
```{r, include = TRUE, eval = TRUE, fig.cap = paste0("Figure ",running_num_figures,": Training History of a Classifier with Prototypes"),fig.align="center"}
...
# 7 Plots for Training History
## 7.1 BaseModels
BaseModels, TEFeatureExtractors, and classifiers provide you with the possibility to create plots
showing the training process of your model. Requesting these plots is similar for all
objects. However, classifiers have the most options for customizing graphics. 
In this chapter we show you how to use the plots. We refer to the models
of the previous chapters in order to give you some examples.

We start with the base models. The arguments `x_min`, `x_max`, `y_min`, and `y_max` allow
you to zoom in our out. If you set these argument to `NULL` the values are determined automatically.

The argument `ind_best_model` allows you to emphasize the epoch with the smallest loss for the
validation sample. In general, this is the state of the model with the best generalization abilities.
In contrast to the classifiers this state is also the final state of the model after training. 
`text_size` allows you to increase or decrease the size of the texts within the graphic. 
...
## 7.2 Classifiers
Classifiers offer a higher number of options. Again with `x_min`, `x_max`, `y_min`, and `y_max` you
can define the area the plot should show. Setting these argument to `NULL` will generate
adequate values automatically. With `measure` you can choose the metric you would like to see.
...
The following plot shows the development of the average iota values during training.
...
If you set `ind_best_model` to `TRUE` you will see some additional points in the graphic.
These point represent the epochs and the corresponding values according to the chosen metric
where the model performed best. In the case of metrics such as the loss the values should be close to 0
while metrics such as accuracy, balanced accuracy, and average iota the best values are close to 1. You can
see one point for every fold during training. 
...
Please note that these point indicate the best states of the model according to the chosen metric in every fold.
However, these values are not always the same values for the final state of the model. The reason is that
the best state of a model during every training loop is determined by the best value for average iota. In the
case that there are multiple states of a model with the same values for average iota the state is selected
with the best value for average iota *and*, in a second step, with the best value for the balanced accuracy. In the case that there are still multiple states the loss is taken into account. That is, possible best states are all
states with the best value for average iota. These states are reduced to the states that have the best value for
balanced accuracy, and these states are reduced to the states with the best values for the loss. From these the
states with  the highest number of epochs is used.

You can add the final states of the model for every fold by setting `ind_selected_model`
to `TRUE`.
...
Since the performance of a model is determined by several folds the minimal and maximal values
allow a deeper understanding of the training process. To add these to the graphic set 
`add_min_max` to `TRUE`. Now the plot shows the complete range of values for every epoch
...
With the argument `final_training` you can switch the plot between the phase of
performance estimation and the final training of the model. Since the final
training phase differentiate only between a training and validation data set there 
is no line for a test data set. In addition, there are no folds. Thus, the argument
...
The last important argument is `pl_step`. This value is only relevant
if the model used pseudo labeling during training. With this parameter you can select
the step during pseudo labeling that should be displayed on the graphic. This works
for the performance estimation as well as for the final training phase. 
