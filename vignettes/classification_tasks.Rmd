...
In our example we use only one encoder layer (`tf_n_layers=1`). All other layers are omitted from the model
by setting the number of layers to zero (`dense_n_layers=0`,`conv_n_layers=0`,`rec_n_layers=0`).
...
test sample should decrease and start increasing after a certain point. 
...
predictions represent the true values.
...
```{r, include = TRUE, eval = TRUE}
# We use t() to improve the readability of the table
t(classifier$reliability$test_metric_mean)
```
...
```{r, include = TRUE, eval = TRUE}
# We use t() to improve the readability of the table
t(classifier$reliability$standard_measures_mean)
```
...
```{r, include = TRUE, eval=TRUE}
# We use t() to improve the readability of the table
t(classifier_prototype$reliability$test_metric_mean)
```

```{r, include = TRUE, eval=TRUE}
# We use t() to improve the readability of the table
t(classifier_prototype$reliability$standard_measures_mean)
```
...
```{r, include = TRUE, eval=TRUE}
# We use t() to improve the readability of the table
t(classifier_with_fe$reliability$test_metric_mean)
```

```{r, include = TRUE, eval=TRUE}
# We use t() to improve the readability of the table
t(classifier_with_fe$reliability$standard_measures_mean)
```
