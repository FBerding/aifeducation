...
In our example we use only two n-gram layers (`ng_conv_n_layers=2`) and 
one dense layers (`dense_n_layers=1`). All other layers are omitted from the model
...
batch size. Their study shows that smaller batch sizes
offer more stable and reliable training. Batch sizes
between 4 and 32 lead to the best performance.

Another important parameter that correlates with batch size is the learning rate (`lr_rate`). The study reported 
by He, Liu et al. (2019) indicates that
that the ratio of batch size to learning rate $(batch size)/(learning rate)$ should be as small as possible in order
to achieve good generalisation performance. 
...
We set `batch_size = 32` and `lr_rate=1e-1` based on the work of He, Liu et al. (2019).
