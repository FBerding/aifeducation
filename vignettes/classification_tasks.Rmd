...
DeBERTa version 2 (He et al. 2020), Funnel-Transformer (Dai et al. 2020), and Longformer
...
Devlin et al. (2019). Download all files into a new folder. Here we store the model in at 'examples/bert_uncased'.
...
a classifier of feature extractor has to deal with. Some of classifier's and
...
data is not directly available for this model on hugging face. If you traing a 
...
The sequential architecture provides models where the input is passed to a specific number 
of layers step by step. All layers are grouped by the kind of layer into stacks.

In contrast, the parallel architecture offers a model where 
...
tries to compress the text embedding to a lower number of dimensions. While this reduces the number
...
As pooling method we use minimum and maximum (`cls_pooling_type="min_max"`). 
...
development of the last training phase set `final_training=FALSE`. If this parameter
is `TRUE` the plot uses the data generated during the performance estimation phase.
...
### 6.1.2 Configuration, Traning, and Application without Sample Data
...
```{r, include = TRUE, eval=TRUE, fig.cap="Figure `r running_num_figures`: Embeddings of an untrained classifier of type 'ProtoNet'"}
...
> and classes of the training data set. These classes and prototypes are used
...
```{r, include = TRUE, eval=TRUE, fig.cap="Figure `r running_num_figures`: Embeddings of a trained classifier of type 'ProtoNet' without unlabeled cases"}
...
```{r, include = TRUE, eval=TRUE, fig.cap="Figure `r running_num_figures`: Embedding of a trained classifier of type 'ProtoNet' including unlabeled cases"}
...
``` {r, include =TRUE, eval=TRUE, fig.cap="Figure `r running_num_figures`: Embeddings of a trained classifier of type 'ProtoNet' trained for two classes and applied for a sample set with four classes." }
...
> It can be beneficial if you use a larger sample of text for training a `TEFeatureExtractor`
...
```{r, include = TRUE, eval=TRUE, fig.cap="Figure `r running_num_figures`: Training History of a Feature Extractor." }
...
model since the aim of the feature extractor is reduced this number. `feat_size`
...
automatically. For example, let us explore the performance of the classifier.
