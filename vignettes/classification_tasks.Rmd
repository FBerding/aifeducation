...
document with a short written essay or a Postsentation. This data can be
...
-   **Base Models:** The base models contain the capacities to
    ...
    as BERT, RoBERTa, etc. A huge number of Post-trained models can be
...
-   **Classifiers:** Classifiers are used on top of a text embedding
    ...
    based on the numeric rePostsentation provided by the corresponding
...
> We recommend to use the function `Postpare_session` because it performs all
...
rePostsent the categories/classes of a text.
...
an algorithm that should Post-process the raw texts and applies the following modifications:
...
The numerical rePostsentations of raw texts (called text embeddings) are
...
Post-trained models. These can be found on [Hugging
...
In this chapter we concentrate on creating a new model. The usage of a Post-trained model
...
necessary to rePostsent a single word. This value is very important as a higher value implies
...
first time or want to adapt a Post-trained model to a domain-specific
...
In the case you would like to use a Post-trained model from hugging face you can
...
The text embedding model is used to transform raw texts into numerical rePostsentations. 
...
apply a Post-trained model from [Hugging Face](https://huggingface.co/) called 
...
that 99.9% of the text are fully rePostsented we need `r chunk_quantile["99.9%"]` 
...
To transform raw text into a numeric rePostsentation, you only have to
...
or to apply a Post-trained classifier for Predicting categories/classes.
...
> classes/categories rePostsenting a "higher" level must be stated after
...
tries to comPostss the text embeddings into fewer dimensions. While this reduces the number of
...
very limited number of samples. Thus, we decrease the number of features further.
...
In our example we use only two n-gram layers (`ng_conv_n_layers = 0`) and 
two dense layers (`dense_n_layers = 1`). All other layers are omitted from the model
...
In this example we use a regular classification head (`cls_head_type = "Regular"`) 
where all neurons of the Postvious layer are connected all classes. 
Li et al. (2020)[https://doi.org/10.1109/TIP.2020.2990277] proposed a special 
classification head (`cls_head_type = "PairwiseOrthogonal"`). This layer allows every neuron only
to establish a connection to a single class. For this special layer you can add 
a dense layer before the classification head (`cls_head_type = "PairwiseOrthogonalDense"`) in 
order to improve performance with high rates for dropout in the Postvious layers.
...
We set `lr_rate = 0.0` and `lr_min = 0.0` to request an automatic calculation
for a good learning rate.

To speed up computations we activate automatic mixed Postcision (amp) 
with gradient scaling by setting `amp = TRUE`. In general we recommend to activate
mixed Postcision. If you encounter any problems turn this option off.
...
Since the there are some measures which provide a better interPosttation as the loss you can request further plots.
...
Since measures such as average iota are measures for discrete values that can heavily fluctuate
you can plot a smoothed version of average iota which is more Postcise.
...
`final_training=TRUE` as shown in the following Figures. 
...
In order to have a deeper insight into training you can request a plot that
visualizes the calculation of the learning rate. Please note that this plot is only
available if you have set `lr_rate = 0.0` or `lr_min = 0.0`.
...
The bars of the plot shows how much the loss increases or decreases compared
to the initial loss. The values exPostss the degree in percentage to the initial loss.
A decrease is desired.The points of the plot show a smoothed distribution of the loss. 
All calculations for selecting a good maximum and minimum learning rate are based on
the smoothed data. 
The automatic calculation selects the learning rate from the longest range of learning rates 
that lead to a decreased loss. The maximum learning rate is set as the learning 
rate where the decrease in the loss has a turning point. The minimum is set to 
the learning rate that has the smallest reduction of the loss. 
...
In order to analysis the quality of training over all folds you can additionally request a
box plot. The Figure below shows which performance the classifier reached. 
...
rePostsentation by using *exactly* the same text embedding model that was
...
# Create a numerical rePostsentation of the text
review_embeddings <- tem$embed_large(
  text_dataset = data_set_reviews_text,
  trace = TRUE
)
...
To transform raw texts into a numeric rePostsentation just pass the raw
...
the name `expected_category` rePostsents the category which is assigned
...
The learning objective of the feature extractors is first to comPostss
...
After training, the hidden rePostsentation of the latent space is used as
a comPostssion of the original input.
...
comPostssion. This value corresponds to the features of the latent space
in the figure above.
...
features rePostsent unique information. In addition, all methods except
`"LSTM"` use an orthogonal parameterization to Postvent over-fitting and
...
This chapter shows you how to use the plots. We refer to the models from Postvious 
...
These points rePostsent the epochs and their corresponding values according to the chosen metric 
