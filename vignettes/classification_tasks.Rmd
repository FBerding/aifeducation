...
> We recommend to use the function `prepare_python` because it performs all
...
Download all files into a new folder. Here we store the model in at 'examples/bert_uncased'.
...
```{r, include = TRUE, eval = FALSE}
load_from_disk("C:/text_embeddings/imdb_movie_reviews")
```
...
![Figure `r running_num_figures`: Basic Architecture of a Sequential Classifier](Model_Sequential.png){width="100%"}
```{r, include = FALSE}
running_num_figures=running_num_figures+1
```
...
![Figure `r running_num_figures`: Basic Architecture of a Parallel Classifier](Model_Parallel.png){width="100%"}
```{r, include = FALSE}
running_num_figures=running_num_figures+1
```
...
That is, the 30 highest and the 30 lowest features are used for calculating
...
![Figure `r running_num_figures`: Embedding of an untrained classifier of type "ProtoNet"](classif_fig_protonet_emb_1.png){width="100%"}
...
![Figure `r running_num_figures`: Embedding of a trained classifier of type "ProtoNet" without unlabeled cases](classif_fig_protonet_emb_2.png){width="100%"}
...
![Figure `r running_num_figures`: Embedding of a trained classifier of type "ProtoNet" including unlabeled cases](classif_fig_protonet_emb_3.png){width="100%"}
...
example_data <- imdb_movie_reviews
example_data$label <- as.character(example_data$label)
example_data$label[c(1:15)] <- "very positive"
example_data$label[c(76:100)] <- NA
example_data$label[c(201:250)] <- NA
example_data$label[c(251:260)] <- "very negative"
...
model although the model was trained only for two classes. Thus, we first have to plsit the data.
...
sample_classes=na.omit(example_data$label)
...
Now we are ready to use our classifier.
