...
example_folders <- setdiff(
  ...
  y = c("examples", "examples/bert_uncased")
)
...
```{r, include = TRUE, eval = TRUE}
chunk_quantile = base_model_bert$Tokenizer$calc_quantiles(
  text_dataset = data_set_reviews_text, 
  batch_size = 32L, 
  seq_len_tokens = 512L,
  token_overlap=128L,
...
```{r, include = TRUE, eval = TRUE}
bert_modeling$BaseModel$count_parameter()
```
...
would be `bert_modeling$BaseModel$get_sustainability_data("training")`.
...
```{r, include = TRUE, eval = FALSE}
bert_modeling$BaseModel$plot_training_history(
  ind_best_model = TRUE
)
```
...
That is, the 25 highest and the 25 lowest features are used for calculating
the classes/labels. The number of values can be determined with `cls_pooling_features=50`.
...
to 50 % or to `r floor(bert_modeling$get_n_features()/2)`.
