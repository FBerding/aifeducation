...
Here you cannot choose the framework (*pytorch* is activated by
default).
...
predicting the texts' labels via classifiers.

To create a collection of raw texts, you have to choose the *Data
Management* page on the top panel as shown in Figure 5.

![Figure 5: Data Management Page](DM_Create.png){width="100%"}

On the resulting page (see Figure 6), you first have to choose the
...
Page](DM_Create_Fill_fields.png){width="100%"}
...
`url_license.txt`, `text_license.txt`, and `url_soruce.text` to every
...
![Figure 7: Structure of Raw Texts within an Excel
Sheet](DM_Create_Excel_file.png){width="75%"}
...
LargeDataSet (*Create Data Set* button on the *Control Panel*) (Figure
8). In the *Directory* field you specify the location where to save the
...
![Figure 8: Selecting a path for a data
set](DM_Create_Start_creation.png){width="100%"}

Finally, you can start creating the collection (*Continue* button in
Figure 8). Please note that this can take some time. You can see the
creation progress in the progress bars (Figure 9).

![Figure 9: Progress of Dataset creation
process](DM_Create_Progress.png){width="100%"}

When the process finishes you can see a message like in Figure 10 (if
the process has completed without any errors).

![Figure 10: Progress of Dataset creation
process](DM_Create_Success.png){width="100%"}
...
If you would like to explore a LargeDataSet for raw texts which is saved on the 
disk you can use the *DataSetExplorer*. The explorer can be found on the tab *Data Management*.
...
Cohan 2020), and MPNet. As a general first step, these models are
...
*Create* on the tab *Base Models* on the top panel of the app. Figure 13
shows the corresponding page.

[![Figure 13: Base Models - Create
Transformer](BM_Create_Fill_fields.png){width="100%"}](BM_Create_Fill_fields.png)
...
*Model-based parameters* of the box *Model Architecture*). Depending on
the architecture, you have different options determining the shape of
your neural network. You also have independent (general) options for all
the transformers (box *General parameters*).

On the right-bottom side of the box *Model Architecture*, you find a box
named *Vocabulary*. Here you must provide the path to a folder which
contains files with data for LargeDataSet. This data set is used to
...
saved (*Choose Folder* button on the *Control Panel*). Finally, you can
start the creation of your model by clicking on the button "Start
Creation". The creation of the model may take some time. You can see the
progress of a creation with progress bars.
...
[![Figure 14: Base Models - Train/Tune
Transformer](BM_Train_Fill_fields.png){width="100%"}](BM_Train_Fill_fields.png)
...
train a model, you must first provide a folder to a dataset (collection
of raw texts) (box *Dataset*). We recommend that you create this
collection of texts as described in section 3.1.1.

Next you can configure the training of your base model (box *Train and
Tune Settings*). Possible options depend on the kind of model.
...
model should be saved after training (*Choose Folder* button on the
*Control Panel*). The corresponding folder will also contain the
...
progress bars and plot (Figure 15).

![Figure 15: Progress of training the
transformer](BM_Train_Progress.png){width="100%"}
...
section "Text Embedding Models". Figure 12 shows the corresponding page.

[![Figure 16: Text Embedding Model -
Create](TEM_Create_Fill_fields.png){width="100%"}](TEM_Create_Fill_fields.png)
...
The tab *Description* (Figure 17) provides you with the documentation of
the model.

[![Figure 17: Text Embedding Model -
Description](TEM_Use_Fill_fields.png){width="100%"}](TEM_Use_Fill_fields.png)
...
[![Figure 18: Text Embedding Model -
Embeddings](TEM_Use_Embedd_text.png){width="100%"}](TEM_Use_Embedd_text.png)
Figure 18: Text Embedding Model - Embeddings (click image to enlarge)

The tab *Tokenize/Encode/Decode* (Figure 19) offers you insights into
...
[![Figure 19: Text Embedding Model -
Tokenize/Encode/Decode](TEM_Use_TED_Encode.png){width="100%"}](TEM_Use_TED_Encode.png)
Figure 19: Text Embedding Model - Tokenize/Encode/Decode (click image to
enlarge)

Finally, the tab *Fill Mask* (Figure 16) allows you to request the
...
[![Figure 20: Text Embedding Model - Fill
Mask](TEM_Use_Fill_mask.png){width="100%"}](TEM_Use_Fill_mask.png)
...
[![Figure 21: Text Embedding Model -
Documentation](TEM_Document_Desc_english.png){width="100%"}](TEM_Document_Desc_english.png)
...
allowing you to reach a broader audience (Figure 21). In all four tabs
...
classifier, click on *Create* in the tab *Classifiers*. Figure 22 shows
the corresponding page.

[![Figure 22: Classifier - Creation Part
1](CF_Create_Fill_fields_1.png){width="100%"}](CF_Create_Fill_fields_1.png)
...
In the third box (*Architecture*) you create the architecture of your
neural network (Figure 23).

[![Figure 23: Classifier - Creation Part
2](CF_Create_Fill_fields_2.png){width="100%"}](CF_Create_Fill_fields_2.png)
...
introduction, we will give you the short version:

-   **Positional Embedding:** By activating this option, you add
    positional embedding to your classifier. This provides the neural
    network with the ability to take into account the order within a
    sequence.

-   **Encoding Layers:** These layers are similar to encoding layers
    used in some transformer models, allowing to calculate
    context-sensitive text embeddings. They provide your classifier with
    the ability to take the surrounding text chunks (see section 3.3.1)
    of the sequences into account.
...
The next box (*Training Settings*) contains the setting for training
your classifier (Figure 24). Going into detail is again beyond the scope
...
[![Figure 24: Classifier - Creation Part
3](CF_Create_Fill_fields_3.png){width="100%"}](CF_Create_Fill_fields_3.png)
...
button *Test Data Matching* (*Control Panel* in Figure 22). This allows
...
In the first tab, *Description* (Figure 25), you will find the
documentation of the model.

[![Figure 25: Classifier -
Description](CF_Use_Fill_fields.png){width="100%"}](CF_Use_Fill_fields.png)
Figure 25: Classifier - Description (click image to enlarge)

In the second tab, *Training* (Figure 26), you receive a summary of the
...
[![Figure 26: Classifier -
Training](CF_Use_Training.png){width="100%"}](CF_Use_Training.png)
Figure 26: Classifier - Training (click image to enlarge)

The third tab, *Reliability* (Figure 27), provides you with the
...
[![Figure 27: Classifier -
Reliability](CF_Use_Reliability.png){width="100%"}](CF_Use_Reliability.png)
Figure 27: Classifier - Reliability (click image to enlarge)

The last tab, *Prediction* (Figure 28), allows you to apply a trained
...
[![Figure 28: Classifier -
Prediction](CF_Use_Prediction_1.png){width="100%"}](CF_Use_Prediction_1.png)
...
![Figure 29: Classifier - Prediction
Results](CF_Use_Prediction_Result.png){width="50%"}
...
Another option to increase the model's performance and/or to increase
...
size can increase model accuracy. In *aifeducation,* a feature extractor
is a model that tries to reduce the number of features of given text
embeddings before feeding the embeddings as input to a classifier.

You can create a feature extractor as follows in Figure 30 (*Create* tab
in the tab *Feature Extractors*).

![Figure 30: Basic architecure of feature
extractors](FE_Create_Fill_fields.png){width="100%"}

First, you need to choose embeddings that you can see after choosing the
box *Input Data* (*Choose Embeddings* button on the *Control Panel*).
You can specify the name and label of the model in the *Model Name* and
*Model Label* fields, respectively.

Specify an architecture and training settings in *Architecture* and
*Training Settings* boxes.

-   *Target Features* determines the number of features for the
...
during configuration of the classifier (Figure 23).
