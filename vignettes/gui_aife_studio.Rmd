...
This article overlaps with vignette [02b Classification
Tasks](classification_tasks.html), which explains how to use the package
...
At the beginning you will see the start page (Figure 4). Here you can
...
the session (box *Machine Learning Framework*). This choice cannot be changed after the session
starts. To change the framework you have to restart *Aifeducation Studio*.

![Figure 4: Start Page](gui_aife_studio_config.jpg){width="100%"}

Depending on the chosen framework, you can activate some further
settings (box *Machine Learning Framework*). If you would like to use tensorflow and your
computer has a graphic device with low memory, we recommend to activate
the option for low memory. For pytorch, no configuration is necessary.

On the right side of the start page, you can decide if energy
consumption should be recorded during training AI (box *Sustainability Tracking*). 
Tracking energy
consumption allows to estimate the CO2 emissions of using AI. Since our
world faces the challenge of climate change, we recommend to enable this
option. In this case you have to choose your country in order to allow a
more accurate estimation of the model's sustainability impact.

If you are ready you can press the start button (box *Start Session*), which directs
...
*Data Preparation* page on the right side as shown in Figure 5. 

![Figure 5: Start Page](gui_aife_studio_home.jpg){width="100%"} 
...
your data with sub-folders. In case you use sub-folders, please ensure
that you include them for creating a collection of raw texts.

![Figure 6: Data Preparation Page](gui_aife_studio_create_text_collection.jpg){width="100%"}

In the next step, you can decide which file formats should be included
(box *File Types*). Currently, *aifeducation* supports .pdf, .csv, and .xlsx
files. If enabled, all files of the requested file format are included
in your data collection.

In case you would like to consider .xlsx files, **all** files must have
one column containing the texts and one column for the texts' IDs as
shown in Figure 7. The name for the corresponding columns must be
identical for all files and you have to provide the column names (first row in
Figure 7).

![Figure 7: Structure of Raw Texts within an Excel Sheet](gui_excel_raw_texts.png){width="50%"}

The last step is to choose a folder where your collection of raw texts
should be saved. Please select a folder and provide a name for your file
(box *Text Output*).

Finally, you can start creating the collection (box *Start Process*). Please note
that this can take some time.

When the process finishes, you have a single file which can be used for
further tasks. The file contains a `data.table` that stores the
...
case of .pdf and .csv files, the file names are used as ID (without the
file extension, see Figure 8).

![Figure 8: Filnames as IDs](gui_excel_raw_texts_ids.png){width="50%"}

> Please note that a consequence of this is that two files *text_01.csv*
...
![Figure 9: Filnames as IDs](gui_excel_target_data.png){width="50%"}
...
Aifeducation Studio supports the opportunitiy to both create and
train/fine-tune base models.

### 3.2.2 Creation of Base Models

In order to create a new base model you have to choose the option
*Create* on the tab *Language Modeling* for the *Base Models* on the right side of the
app (Figure 5). 

[![Figure 10: Language Modeling - Create Transformer](gui_aife_studio_create_transformer.jpg){width="100%"}](gui_aife_studio_create_transformer.jpg)
Figure 10: Language Modeling - Create Transformer (click image to enlarge)
...
transformer architectures (box *Model Architecture*). Depending on the architecture, you
have different options determining the shape of your neural network.

In the middle, you find a box named *Vocabulary*. Here you must provide
the path to a file which contains a collection of raw texts. These raw
texts are used to calculate the vocabulary of the transformer.
This file should be created with *Aifeducation Studio* to ensure
...
saved (box *Creation*). Finally, you can start the creation of your model by
...
[![Figure 11: Language Modeling - Train/Tune  Transformer](gui_aife_studio_train_transformer.jpg){width="100%"}](gui_aife_studio_train_transformer.jpg)
Figure 11: Language Modeling - Train/Tune  Transformer (click image to enlarge)
...
train a model, you must first provide a collection of raw texts (box *Raw Texts*).
...
model should be saved after training (box *Start Training/Tuning*). The corresponding folder
...
amount of data, and the capacities of your machine.
...
ability to understand natural language. You can open the creation page
by clicking on "Create" in the section "Text Embedding Model" 
via "Language Modeling" (see Figure 5).

[![Figure 12: Text Embedding Model - Create](gui_aife_studio_emb_interface_create.jpg){width="100%"}](gui_aife_studio_emb_interface_create.jpg)
...
-   **Aggregation Hidden States**: Base models transform raw data into a
    sequence of numbers. With this option you can decide how these
    numbers are generated. In general, the hidden states of the last
    layer are used as a numeric representation.
...
The tab *Model Description* provides you with the documentation of the
model.

[![Figure 13: Text Embedding Model - Description](gui_aife_studio_emb_interface_use_desc.jpg){width="100%"}](gui_aife_studio_emb_interface_use_desc.jpg)
Figure 13: Text Embedding Model - Description (click image to enlarge)
 
The tab *Create Text Embeddings* allows you to transform raw texts into
...
[![Figure 14: Text Embedding Model - Embeddings](gui_aife_studio_emb_interface_use_embed.jpg){width="100%"}](gui_aife_studio_emb_interface_use_embed.jpg)
Figure 14: Text Embedding Model - Embeddings (click image to enlarge)

The tab *Encode/Decode/Tokenize* offers you insights into the way your
...
[![Figure 15: Text Embedding Model - Encode/Decode/Tokenize](gui_aife_studio_emb_interface_use_encode_decode.jpg){width="100%"}](gui_aife_studio_emb_interface_use_encode_decode.jpg)
Figure 15: Text Embedding Model - Encode/Decode/Tokenize (click image to enlarge)

Finally, the tab *Fill Mask* allows you to request the underlying base
...
[![Figure 16: Text Embedding Model - Fill Mask](gui_aife_studio_emb_interface_use_fill_mask.jpg){width="100%"}](gui_aife_studio_emb_interface_use_fill_mask.jpg)
...
[![Figure 17: Text Embedding Model - Documentation](gui_aife_studio_emb_doc.jpg){width="100%"}](gui_aife_studio_emb_doc.jpg)
...
allowing you to reach a broader audience. In all four tabs you can
...
[![Figure 18: Classifier - Creation Part 1](gui_aife_studio_classifier_create_part_1.jpg){width="100%"}](gui_aife_studio_classifier_create_part_1.jpg)
...
[![Figure 19: Classifier - Creation Part 2](gui_aife_studio_classifier_create_part_2.jpg){width="100%"}](gui_aife_studio_classifier_create_part_2.jpg)
...
-   **Calculate Baseline Model:** If active, the performance of a
    baseline model is estimated. This does not include *Balanced Pseudo
    Labeling* and *Balanced Synthetic Cases*.
...
-   **Add Synthetic Cases:** If active, the creation of additional
    ...
    -   **Number of Cores:** The number of cores which can be used for
        generating the synthetic cases. A higher number can speed up the
        training process.
    -   **Method:** The method which should be used for generating the
        cases.
    -   **Max k:** The maximum number of neighbors used for generating
        the synthetic cases. The algorithm will create cases up to this
        and draw a random sample from it.
    -   **Proportion for Validation Sample:** The percentage of
        synthetic cases which should be added to the validation sample
        instead of the training sample.
-   **Add All Synthetic Cases:** If enabled, all synthetic cases are
    added. If disabled, only the given number of cases is added to the
    sample to ensure a balanced frequency of all classes/categories.

**Section: Balanced Pseudo-Labeling**

-   **Add Pseudo Labeling:** If activated, pseudo-labeling is used
    during training. The way pseudo-labeling is applied can be
    ...
    -   **Balance Pseudo-Labels:** If this option is active, the same
        number of pseudo-labeled cases is added for every
        class/category. In general, this number is determined by the
        class with the smallest absolute frequency.
    ...
    -   **Reset Model After Every Step:** If enabled, the classier is
        set to an untrained state. This can prevent over-fitting.

    -   **Dynamic Weight Increase**: If enabled, the sample weights of
        the pseudo-labeled cases increase with every step. The weights
        are determined with *Start Weights* and *Weight Increase per
        Step*.

    -   **Start Weights:** Initial value for the sample weights of the
        included pseudo-labeled cases.

    -   **Weight Increase per Step:** Value determining how much the
        sample weights are increased for the included pseudo-labeled
        cases in every step.
...
[![Figure 20: Classifier - Creation Part 3](gui_aife_studio_classifier_create_part_3.jpg){width="100%"}](gui_aife_studio_classifier_create_part_3.jpg)
...
In the first tab, *Model Description,* you will find the documentation
of the model.

[![Figure 21: Classifier - Description](gui_aife_studio_classifier_use_desc.jpg){width="100%"}](gui_aife_studio_classifier_use_desc.jpg)
Figure 21: Classifier - Description (click image to enlarge)

In the second tab, *Training,* you receive a summary of the training
...
[![Figure 22: Classifier - Training](gui_aife_studio_classifier_use_train.jpg){width="100%"}](gui_aife_studio_classifier_use_train.jpg)
Figure 22: Classifier - Training (click image to enlarge)

The third tab, *Reliability,* provides you with information on the
...
[![Figure 23: Classifier - Reliability](gui_aife_studio_classifier_use_reliability.jpg){width="100%"}](gui_aife_studio_classifier_use_reliability.jpg)
Figure 23: Classifier - Reliability (click image to enlarge)

The last tab, *Prediction,* allows you to apply a trained model to new
...
[![Figure 24: Classifier - Prediction](gui_aife_studio_classifier_use_predictions.jpg){width="100%"}](gui_aife_studio_classifier_use_predictions.jpg)
...
[![Figure 25: Classifier - Prediction Results](gui_aife_studio_classifier_predictions.jpg){width="100%"}](gui_aife_studio_classifier_predictions.jpg)
