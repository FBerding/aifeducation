...
This vignetteintroduces *aifeducation - Studio* which is a graphical
...
with *R* or thosw who do not have coding skills in relevant languages
...
CentralConcepts](classif_fig_01.png){width="100%"}
...
-   **Classifiers:** Classifiers are used on top of a text embedding
    ...
    based on the numeric representation provided be the corresponding
...
the session (point 1). This choice cannot be changed after the session
starts. To change the framework you have to restart *Aifeducation
Studio*.

![Figure 4: Start Page](gui_start_page.png){width="100%"}

Depending on the chosen framework, you can activate some further
settings (point 2). If you would like to use tensorflow and your
computer has a graphic device with low memory, we recommend to activate
the option for low memory. For pytorch, no further changes are
necessary.

On the right side of the start page, you can decide if energy
consumption should be recorded during training AI. Tracking energy
...
If you are ready you can press the start button (point 3), which directs
...
To create a collection of raw text, you have to choose the data
preparation page on the right side as shown in Figure 5. ![Figure 5:
Start Page](gui_home_page_data_preparation.png){width="100%"} On the
resulting page, you first have to choose the directory where the texts
are stored (point 1). We recommend that you store all texts you would
...
that you include them for creating a collection of raw texts (point 2).

In the next step, you can decide which file formats should be included
(point 3). Currently, *aifeducation* supports .pdf, .csv, and .xlsx
...
shown in Figure 6. The name for the corresponding columns must be
identical for all files and you have to provide the name (point 4).

![Figure 6: Structure of Raw Texts within an Excel
Sheet](gui_excel_raw_texts.png){width="50%"}
...
Finally, you can start creating the collection (point 6). Please note
that this can take some time.

When the process finishes, you have a single file which can be used for
further tasks. The file contains a data.table that stores the ids of the
...
corresponding label if available.
...
.rda/rdata. Figure 8 illustrates an example for a .xslx file.
...
"Create" on the tab "Masked Language Modeling" on the right side of the
app (Figure 9).

![Figure 9: Start Page](gui_home_page_lm_create.png){width="100%"} Every
transformer model is composed of two parts: 1) the tokenizer which
splits raw text into smaller pieces to model a large number of words
...
transformer architectures (point 1). Depending on the architecture, you
...
texts are used to calculate the vocabulary of the transformer (point 3).
...
include (point 4). Depending on the transformer method, you can set
additional options affecting a transformer's vocabulary (point 5).
...
saved (point 6). Finally, you can start the creation of your model by
...
train/tune. Please note that every base model consists of several files.
...
train a model, you must first provide a collection of raw texts (point
1). We recommend that you create this collection of texts as described
in section 3.1.1.

![Figure 11: Start
Page](gui_home_page_lm_train_mlm_full.png){width="100%"}

Next you can configure the training of your base model (point 2):
...
model should be saved after training (point 3). The corresponding folder
...
by clicking on "Interface" via "Language Modeling" (see Figure 12).
...
the entire model and not single files.

After choosing a model, new boxes appear which allow you to customize
your interface (point 1 in Figure 13). Here it is very important that
...
new model, we recommend to use "0.1". For *Language,* it is necessary
...
On the right side of the box (point 2) you can set how the interface
...
have to select a collection of raw texts (point 1 in Figure). We
...
stored and name it (point 2). With *Batch Size* you can determine how
...
refers to the peple who adapted a pre-trained model to another domain or
task.
...
classifier, click on "Create and Train" in the tab *Classification*.
...
the first box (point 1). After selecting the embeddings, you will see a
...
In the second box, you can select the file that contains the
corresponding labels (point 2). After loading the file, you can select
...
In the third box you create the architecture of your neural network. It
is very important that you provide the model's name and label in the
...
-   **Recurrent Layers:** This section allows you to add recurrent
    ...
    determines the number of neurons. Below the field you can see a what
...
The next box contains the setting for training your classifier. Going
into detail is again beyond the scope of this introduction. Here we can
...
In the last box, you have to provide the directory where you would like
to save model. The name of the folder created within that directory can
...
button *Test Data Matching*. This allows you to check if the structure
