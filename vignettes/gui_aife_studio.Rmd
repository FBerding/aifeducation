...
*AI for Education - Studio* by entering the following into the console:
...
Figure `r running_num_figures` shows an example.
...
Within the box *Text Processing* you can activate an algorithm that cleans
the text stored in all .txt. and .pdfs files. If you active this option creating the
data set changes the texts in the following aspects:
...
like creating and training/tuning the transformers (*Base Models* tab on
...
Funnel-Transformer (Dai et al. 2020), and MPNet (Song et al. 2020). 
As a general first step, these models are
...
[Hugging Face](https://huggingface.co/). Sometimes, however, it is more
straightforward to create a new model to fit a specific purpose.
*AI for Education - Studio* supports the opportunity to both create and
...
At the beginning you find a box named *Input Data* at the top. Here you must provide the path to a folder which
...
with **AI for Education - Studio* (tab *Data Management*) to ensure
compatibility. See section 3.1.1 for more details. 

In the next step you can choose between the different types of tokenizer 
(*Tokenizer Model Type* combo box in *Control Panel*, left side). Depending on the model
type you have different options for configuration. On very important parameter is the
size of the vocabulary. With this value you determine how many tokens should be used
to split words and sentences into smaller pieces.

Depending on the tokenizer method, you can set additional
options, affecting raw text processing.
...
If you have set all parameters to the desired values you can click on the button 
*Calculate Statistics* to receive the tokenizer granularity rate This value
describes how many tokens are in average necessary to represent a single word. This value 
is important as it has an impact on the necessary length of a sequence of tokens for modeling a
text. A tokenizer with a higher value needs a longer sequence of tokens for representing the same text
as a tokenizer with a smaller value. since most transformer have a maximum limit for the length
of a token sequence this values has directly an impact on the base model.

The final step is to configure the transformer model. You can choose the between
different architectures (*Base Model Type* combo box in *Control Panel*, left side). 
...
*AI for Education - Studio*. In addition you can use any model from
[Hugging Face](https://huggingface.co/) that uses an architecture
...
After selecting a base model and a data set you can calculate some statistics.
You can click on the button *Calculate Data Statistics*. This will provide you
an estimate on the total number of words, tokens, and the tokenizer granularity rate
for the specific data set and model.

In addition you can click on the button *Calculate FLOPS Statistics*. This will 
give you an estimate on the total number of FLOPS for the planned training run. 
That is, this value depends on the size for the training data, model's architecture and
...
-   **Minimal Sequence Length:** For training and validating a base model, the raw
...
capacities of your machine. You can see the training progress with 
progress bars and a plot (Figure `r running_num_figures`).
...
The text embedding model is the model that transforms raw texts into a numerical 
representation. In
...
long documents by applying the same algorithm to different parts of a long document.
...
your interface. Here you can give your model a label (*Control Panel*). 
Think of *Label* as the title of
...
-   **N chunks:** Sometimes texts are very long. With this value, you
    ...
    "Maximum Sequence Length". The number of chunks must be at least two.
...
-   **Layers for Embeddings - Min**: Text embedding models transform raw data into
...
$$Tokens_{Max} = N_{Chunks}*MaximalSequenceLength-(N_{Chunks}-1)*N_{Overlap}$$ 

If the text is longer, the remaining tokens are ignored and lost for
further analysis. The result of this calculation is shown on the right side allowing 
you to experiment with different options. In the case that the base model comes
with tokenizer statistics the number of tokens is replaced with an estimate
of the total number of words for a simpler interpretation. All values
are estimates. That is, the real number of words/tokens for a given text depends
...
have to select a data set containing raw texts. W
e recommend that you create this collection according to
section 3.1.1 in order to ensure compatibility. 

Next you have to provide the folder where the embeddings
...
the way your text embedding model processes texts. In the box *Encode*
...
Meaningful documentation is required. In addition, a well written
...
To support developers in documenting their work, *AI for Education - Studio*
...
allowing you to reach a broader audience (Figure `r running_num_figures-1`). In all four tabs
...
In contrast, the parallel architecture (see Figure `r running_num_figures`) offers a model where 
...
to build a high number of different models (see box *Model Configuration* in Figure `r running_num_figures`).
...
cases of the majority class. The synthetic cases for every class are
...
-   **Add Pseudo Labeling:** If activated, pseudo-labeling is used
    ...
    -   **Max Steps:** The number of steps during pseudo-labeling. For
        ...
        *Certainty Anchor*, *Max Certainty Value*, and
        *Min Certainty Value*.
...
button *Test Data Matching* (*Control Panel* in Figure `r running_num_figures-5`). This allows
...
*AI for Education - Studio* asks you to select the folder containing these
...
epoch. Depending on the applied training techniques (such as *Pseudo-Labeling*), 
...
classifier. Just select the feature extractor during configuration of a 
classifier (Figure `r first_classifier_image`).
...
shows an example for exploring the training and validation loss. 

[![Figure `r running_num_figures`: Feature Extractor - Training](FE_Use_Training.png){width="100%"}](FE_Use_Training.png)
Figure `r running_num_figures`: Feature Extractor - Training (click image to enlarge)
