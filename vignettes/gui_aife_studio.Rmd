...
(*Create Data Set* button on the *Control Panel*) (Figure 8). With *Directory* field you
...
![Figure 8: Selecting a path for a data set](img_articles/gui/DM_Create_Start_creation.png){width="100%"}

Finally, you can start creating the collection (*Continue* button in Figure 8). Please note
that this can take some time. Creation progress you can see in the progress bars (Figure 9).

![Figure 9: Progress of Dataset creation process](img_articles/gui/DM_Create_Progress.png){width="100%"}

When the process finishes you can see message like in Figure 10 (in the case if
the process has completed without any errors).

![Figure 10: Progress of Dataset creation process](img_articles/gui/DM_Create_Success.png){width="100%"}
...
file extension, see Figure 11).

![Figure 11: Filnames as IDs](img_articles/gui/DM_Create_Text_sources.png){width="75%"}
...
.rda/.rdata. Figure 12 illustrates an example for a .xslx file.  

![Figure 12: Filnames as IDs](img_articles/gui_excel_target_data.png){width="50%"}
...
app. Figure 13 shows the corresponding page.

[![Figure 13: Base Models - Create Transformer](img_articles/gui/BM_Create_Fill_fields.png){width="100%"}](img_articles/gui/BM_Create_Fill_fields.png)
Figure 13: Base Models - Create Transformer (click image to enlarge)
...
[![Figure 14: Base Models - Train/Tune  Transformer](img_articles/gui/BM_Train_Fill_fields.png){width="100%"}](img_articles/gui/BM_Train_Fill_fields.png)
Figure 14: Base Models - Train/Tune Transformer (click image to enlarge)
...
After choosing a base model, new boxes appear as shown in Figure 14. To
...
progress with the progress bars and plot (Figure 15).

![Figure 15: Progress of training the transformer](img_articles/gui/BM_Train_Progress.png){width="100%"}
...
by clicking on "Create" tab in the section "Text Embedding Models". Figure 12 shows the corresponding page.

[![Figure 16: Text Embedding Model - Create](img_articles/gui/TEM_Create_Fill_fields.png){width="100%"}](img_articles/gui/TEM_Create_Fill_fields.png)
Figure 16: Text Embedding Model - Create (click image to enlarge)
...
the entire model and not single files (button *Choose a Base Model* on the *Control Panel*).

After choosing a model, new boxes appear which allow you to customize
your interface. Here it is very important that
you give your model a unique name and label (*Control Panel*). The difference between
...
the title of a book or paper. With *Label* you can also provide a version
number if you create a newer version of your model. In case you create a
new model, we recommend to use "0.0.1". Fo    essary
...
-   **Pooling Type**: With this option you can decide if only the hidden states of
    the cls-token should be used for the embedding. If you set this option to
...
clicking on "Use" in the tab "Text Embedding Models". At the start you have
to choose the model that you would like to use. Please select the folder
that contains the entire model instead of selecting single files (button *Choose a Model*). After
selecting and loading a model, a new box appears that shows you the
different aspects of the model and how you can use it.

The tab *Description* (Figure 17) provides you with the documentation of the
model.

[![Figure 17: Text Embedding Model - Description](img_articles/gui/TEM_Use_Fill_fields.png){width="100%"}](img_articles/gui/TEM_Use_Fill_fields.png)
Figure 17: Text Embedding Model - Description (click image to enlarge)
...
The tab *Embedd Text* (Figure 18) allows you to transform raw texts into
...
have to select a collection of raw texts (a folder containing several files). We
...
value to your machine's capacities. By clicking on the button "Save Embeddings"
the transformation of texts begins.

[![Figure 18: Text Embedding Model - Embeddings](img_articles/gui/TEM_Use_Embedd_text.png){width="100%"}](img_articles/gui/TEM_Use_Embedd_text.png)
Figure 18: Text Embedding Model - Embeddings (click image to enlarge)

The tab *Tokenize/Encode/Decode* (Figure 19) offers you insights into the way your
...
[![Figure 19: Text Embedding Model - Tokenize/Encode/Decode](img_articles/gui/TEM_Use_TED_Encode.png){width="100%"}](img_articles/gui/TEM_Use_TED_Encode.png)
Figure 19: Text Embedding Model - Tokenize/Encode/Decode (click image to enlarge)
...
insert a gap/mask_token please ensure correct spelling. With 
*N Solutions per mask* you can determine how many tokens the model should
...
[![Figure 20: Text Embedding Model - Fill Mask](img_articles/gui/TEM_Use_Fill_mask.png){width="100%"}](img_articles/gui/TEM_Use_Fill_mask.png)
Figure 20: Text Embedding Model - Fill Mask (click image to enlarge)
...
part of the app by clicking on "Document" in the tab *Text Embedding Models*. 
First, you have to choose the text embedding model you would
...
[![Figure 21: Text Embedding Model - Documentation](img_articles/gui/TEM_Document_Desc_english.png){width="100%"}](img_articles/gui/TEM_Document_Desc_english.png)
Figure 21: Text Embedding Model - Documentation (click image to enlarge)
...
allowing you to reach a broader audience (Figure 21). In all four tabs you can
...
classifier, click on *Create* in the tab *Classifiers*.
Figure 22 shows the corresponding page.

[![Figure 22: Classifier - Creation Part 1](img_articles/gui/CF_Create_Fill_fields_1.png){width="100%"}](img_articles/gui/CF_Create_Fill_fields_1.png)
Figure 22: Classifier - Creation Part 1 (click image to enlarge)
...
the first box (*Input Data*) by clicking on *Choose Embeddings* button. After selecting the embeddings, you will see a
...
It is very important that you provide the model's name and label in the
*Control Panel*. The *Model Name* is used for internal purposes by the
...
[![Figure 23: Classifier - Creation Part 2](img_articles/gui/CF_Create_Fill_fields_2.png){width="100%"}](img_articles/gui/CF_Create_Fill_fields_2.png)
Figure 23: Classifier - Creation Part 2 (click image to enlarge)

Since a detailed explanation of every option is beyond the scope
...
(Figure 24). Going into detail is again beyond the scope of this introduction. Here we can
...
-   **Max Steps:** The number of steps during pseudo-labeling. For
    ...
    added, etc.. Which cases are added can be influenced by 
    *Balance Pseudo-Labels*, *Certainty Anchor*, *Max Certainty Value*, and
    *Min Certainty Value*.
...
[![Figure 24: Classifier - Creation Part 3](img_articles/gui/CF_Create_Fill_fields_3.png){width="100%"}](img_articles/gui/CF_Create_Fill_fields_3.png)
Figure 24: Classifier - Creation Part 3 (click image to enlarge)
...
button *Test Data Matching* (*Control Panel* in Figure 22). This allows you to check if the structure
of the data is working. If everything is okay you can provide the directory where you would like
to save the model and start training the
model by clicking on *Train Model*.

Please note that training a
classifier can take up to several hours. You can see a progress on the progress bars.
...
to classify new texts. To do so, you have to select *Use* tab in the tab
*Classifiers*.
...
In the first tab, *Description*, (Figure 25) you will find the documentation
of the model.

[![Figure 25: Classifier - Description](img_articles/gui/CF_Use_Fill_fields.png){width="100%"}](img_articles/gui/CF_Use_Fill_fields.png)
Figure 25: Classifier - Description (click image to enlarge)

In the second tab, *Training* (Figure 26) you receive a summary of the training
...
[![Figure 26: Classifier - Training](img_articles/gui/CF_Use_Training.png){width="100%"}](img_articles/gui/CF_Use_Training.png)
Figure 26: Classifier - Training (click image to enlarge)

The third tab, *Reliability*, (Figure 27) provides you with information on the
...
[![Figure 27: Classifier - Reliability](img_articles/gui/CF_Use_Reliability.png){width="100%"}](img_articles/gui/CF_Use_Reliability.png)
Figure 27: Classifier - Reliability (click image to enlarge)

The last tab, *Prediction* (Figure 28) allows you to apply a trained model to new
...
[![Figure 28: Classifier - Prediction](img_articles/gui/CF_Use_Prediction_1.png){width="100%"}](img_articles/gui/CF_Use_Prediction_1.png)
Figure 28: Classifier - Prediction (click image to enlarge)
...
predictions in an .csv file. The resulting data table may look like shown in Figure 29.

![Figure 29: Classifier - Prediction Results](img_articles/gui/CF_Use_Prediction_Result.png){width="50%"}

### 3.4.3 Documenting a Classifier

Documenting a classifier is similar to the documentation of a text
embedding model (section 3.3.3, see Figure 22).
...
You can create a feature extractor as follows in Figure 30 (*Create* tab in the tab *Feature Extractors*).

![Figure 30: Basic architecure of feature extractors](img_articles/gui/FE_Create_Fill_fields.png){width="100%"}

First, you need to choose embeddings that you can see after choosing in box *Input Data*
(*Choose Embeddings* button on the *Control Panel*). You can specify name and label of the model in *Model Name* and *Model Label* fields respectively.

Specify an architecture and training settings in *Architecture* and *Training Settings* boxes.

-   *Target Features* determines the number of features for the compressed representation. The
...
If set "lstm" all layers of the model are long short-term memory layers.
If set "dense" all layers are standard dense layers. If you set "conv"
all layers are convolutional layers.

-   With *Noise Factor* you can add some noise during training making the feature extractor
to an denoising auto encoder which can provide more robust generalizations.

When you are ready,
to start training just click on the *Train Model* button. Then you have to choose a path to a folder and a folder name, where you want to store the model.

Please note that training a
classifier can take up to several hours. You can see a progress on the progress bars.

After you have trained your feature extractor you can use it for every classifier.
Just pass the feature extractor path to *Feature Extractor* during configuration of the
