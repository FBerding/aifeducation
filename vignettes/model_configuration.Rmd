...
A very important hyperparameter is `batch_size`. It sets the number of cases to be processed
...
Therefore, we recommend using low batch sizes when training a model. If you would 
like to use a prototype-based classifier, the rules for contrastive learning should 
be applied, since these models use a contrastive loss function. In this case, 
the batch size is determined by two parameters: `Ns` and `Nq.` Set these two parameters to high values.
...
to achieve good generalisation performance. If you use `batch_size=4` we recommend to set `lr_rate=1e-1`.
