...
-   **Transformers:** Transformers are the current state-of-the-art
    ...
    embeddings (Chollet, Kalinowski & Allaire, 2022 pp.366). In
    *aifeducation*, only the architecture of BERT is implemented as the
...
suggested in vignette [01 Get started](articles/aifeducation.html) you
...
Now is a good time to configure tensorflow, since some configurations
...
than the vocabularies built in section 2.2.
...
the usage of memory with `set_config_gpu_low_memory()`.
...
Finally, you have to decide if you would like the embedding of the
classification token [CLS] as text embedding or the mean of all token
embeddings as text embedding (`use_cls_token`). You can further decide
from which hidden layer or layers the embeddings should be drawn
(`aggregation="last"`). In their initial work, Devlin et al. (2019) used
...
Please note that in the case of BERT models, the embeddings of every
...
GlobalVectorClusters on the one hand and models based on transformers on
the other hand.
...
models serve as an interface to *R*. The original model is saved to a
model directory. Thus, you have to save your interface to that very
model directory.
...
used for training your classifier. The resulting object can then be passed to
the method `predict` and you will get the predictions together with an
...
Research to Practice. Springer New York.
...
Topic-Modellen: Eine Einführung für Sozialwissenschaftler. Springer.
...
with hugging face (Revised edition). O'Reilly.
