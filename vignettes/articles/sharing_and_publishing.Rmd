...
*aifeducation* is based on the assumption that AI-based classifiers
...
more easy. You can write your description in HTML which allows you to
...
description to allow a wider community to recognize xour work**.
...
to add. You have three choices: "developer", "trainer", and "modifier",
...
[Huggingface](https://huggingface.co/), the people and their description
...
Sometimes the personas who developed a model and the people who trained
a model differ. In these cases you can pay credit to the individuals who
trained a model via setting `type="trainer"`.
...
native language of your classifier processes. With `abstract_eng` and
...
keywords_eng="Self-determination theory, motivation, lesson planning, business didactics",
keywords_native="Selbstbestimmungstheorie, Motivation, Unterrichtsplanung, Wirtschaftsdidaktik")
```
...
-   A short reference to the theoretical models that guided the
    development.
-   A clear and detailed description of every single category/class and
    its development.
...
Please use "developer" and "trainer" if you are the first person who
trained the classifier. `type="modifier"` will be rarely needed.

Finally, you can manage the license for using your classifier via
`set_license` and `get_license`.
...
tasks](articles/classification_tasks.html).
