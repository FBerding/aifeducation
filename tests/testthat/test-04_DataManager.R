  ...
  for(fold in folds){
    ...
    for(i in 1:(test_datamanager$get_n_folds()+1)){
      ...
      test_that(paste("DataManager - Pseudo Data","Fold:",i),{
        ...
        expect_equal(object=length(test_datamanager$datasets$data_labeled_pseudo),
                     expected=10)
      })
      gc()
      #----------------------------------------------------------------------------
      test_that(paste("DataManager - get_dataset()","Fold:",i),{
        data_test=test_datamanager$get_dataset(inc_labeled = TRUE,
                                               inc_unlabeled = FALSE,
                                               inc_synthetic = TRUE,
        ...
        data_test=test_datamanager$get_dataset(inc_labeled = FALSE,
                                               inc_unlabeled = FALSE,
                                               inc_synthetic = TRUE,
                                               inc_pseudo_data = TRUE)
        if(!is.null(data_test)){
          ...
          expect_equal(object = test_datamanager$datasets$data_labeled_synthetic,
                       expected = NULL)
        }
