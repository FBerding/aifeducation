...
testthat::skip_if_not(
  condition = file.exists(testthat::test_path(path)),
        ...
        for (r in r_encoder_list) {
          for (attention in attention_list) {
            for (pos_embedding in pos_embedding_list) {
              test_that(paste(
                framework, "creation_classifier_neural_net", "n_classes", n_classes,
                "rec", paste(rec, collapse = "_"), "hidden", paste(hidden, collapse = "_"),
                "encoder", r, "attention", attention, "pos", pos_embedding
              ), {
                classifier <- NULL
      ...
      test_that(paste(framework, "Saving Classifier pytorch", n_classes), {
        ...
        if (reticulate::py_module_available("safetensors")) {
          expect_true(file.exists(testthat::test_path(
            paste0("test_artefacts/tmp_pytorch/", n_classes, "_classes", "/model_data.safetensors")
          )))
        } else {
          expect_true(file.exists(testthat::test_path(
            paste0("test_artefacts/tmp_pytorch/", n_classes, "_classes", "/model_data.pt")
          )))
        }
      })
    ...
    if (framework == "tensorflow") {
      ...
      test_that(paste(framework, "Classifier Load Total Model keras_V3 without ID"), {
        ...
        new_classifier <- load_ai_model(
          ml_framework = "tensorflow",
          model_dir = testthat::test_path(
            paste0("test_artefacts/tmp_full_models_keras/", classifier$get_model_info()$model_name_root)
          )
        )
      ...
      test_that(paste(framework, "Classifier Load Total Model keras_V3 with ID"), {
        ...
        new_classifier <- load_ai_model(
          ml_framework = "tensorflow",
          model_dir = testthat::test_path(
            paste0("test_artefacts/tmp_full_models_keras/", classifier$get_model_info()$model_name)
          )
        )
        expect_s3_class(new_classifier,
          class = "TextEmbeddingClassifierNeuralNet"
      ...
      test_that(paste(framework, "Classifier Load Total Model TF with ID"), {
        ...
        new_classifier <- load_ai_model(
          ml_framework = "tensorflow",
          model_dir = testthat::test_path(
            paste0("test_artefacts/tmp_full_models/", classifier$get_model_info()$model_name)
          )
        )
        expect_s3_class(new_classifier,
          class = "TextEmbeddingClassifierNeuralNet"
      ...
      test_that(paste(framework, "Classifier Load Total Model TF without ID"), {
        ...
        new_classifier <- load_ai_model(
          ml_framework = "tensorflow",
          model_dir = testthat::test_path(
            paste0("test_artefacts/tmp_full_models/", classifier$get_model_info()$model_name_root)
          )
        )
        expect_s3_class(new_classifier,
          class = "TextEmbeddingClassifierNeuralNet"
      ...
      test_that(paste(framework, "Classifier Load Total Model pytorch without ID"), {
        ...
        new_classifier <- load_ai_model(
          ml_framework = "pytorch",
          model_dir = testthat::test_path(
            paste0("test_artefacts/tmp_full_models_pytorch/", classifier$get_model_info()$model_name_root)
          )
        )
        expect_s3_class(new_classifier,
          class = "TextEmbeddingClassifierNeuralNet"
